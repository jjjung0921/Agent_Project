# ADR-0003: AGENTS.md 규칙·절차 분리와 컨텍스트 예산
...
ADR-0002까지 반영한 시점의 `AGENTS.md`는 139줄, 규칙성 문장 70개였고, 세션 시작 시 필수로 읽는 컨텍스트(AGENTS + CURRENT + HANDOFF + 현재 PLAN + 스크립트 출력)는 약 18KB였다. 토큰 양으로는 200K 컨텍스트의 3~4%라 문제가 아니지만, 세 가지 희석 요인이 있었다.
