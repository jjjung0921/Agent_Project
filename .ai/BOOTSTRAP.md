...
1. `AGENTS.md`를 읽는다. 그 규칙은 이 절차에도 적용된다 (특히 Change Tracking, Commit Policy, Scope Discipline).
...
4. 스택·핵심 구조 결정을 `docs/decisions/ADR-0003-*.md`부터 기록한다 (`_template.md` 사용; 0001·0002는 이 템플릿의 협업 구조 결정이므로 유지). 사소한 결정은 ADR로 만들지 않는다.
...
7. 이 파일(`.ai/BOOTSTRAP.md`)을 삭제하고, `README.md`·`.ai/CURRENT.md`·Phase 01 PLAN에서 BOOTSTRAP 참조를 제거한다.
8. 작업 커밋을 남긴다: `chore: bootstrap project from template` (Commit Policy대로 `Agent:`·`Task: 01/T1` trailer 포함).
9. `.ai/LOG.md`에서 템플릿 생성 세션 항목을 지우고, `AGENTS.md`의 End of Work 절차대로 종료한다 (CURRENT.md·HANDOFF.md 갱신, LOG 항목 추가, `scripts/ai-end.sh --set-checkpoint`, close commit).
