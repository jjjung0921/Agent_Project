...
1. `AGENTS.md`를 읽는다. 그 규칙은 이 절차에도 적용된다 (특히 Rule 4 Developer changes, 6 Scope, 9 Commits).
...
4. 스택·핵심 구조 결정을 `docs/decisions/`의 다음 번호 ADR부터 기록한다 (`_template.md` 사용; 기존 ADR은 이 템플릿의 협업 구조 결정이므로 유지). 사소한 결정은 ADR로 만들지 않는다.
...
8. 작업 커밋을 남긴다: `chore: bootstrap project from template` (Commit Format대로 `Agent:`·`Task: 01/T1` trailer 포함).
9. `.ai/LOG.md`에서 템플릿 생성 세션 항목을 지우고, `AGENTS.md`의 Session Procedure(종료)대로 끝낸다 (CURRENT.md·HANDOFF.md 갱신, LOG 항목 추가, `scripts/ai-end.sh --set-checkpoint`, close commit).
